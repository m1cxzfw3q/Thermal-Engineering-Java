...
public class MultiCrafter extends PayloadBlock {
        ...
        public float calculateHeat(float[] sideHeat) {
            ...
            for (Point2 edge : edges) {
                ...
                if (build != null && build.team == this.team && build instanceof HeatBlock heater) {
                    ...
                    if (heater instanceof MultiCrafterBuild multi) {
    ...
    protected void buildIOEntry(Table table, Recipe recipe, boolean isInput) {
        ...
        for (LiquidStack stack : entry.fluids) {
            Cell<TEMod.MultiCraftLib.ui.FluidImage> iconCell = mat.add(new TEMod.MultiCraftLib.ui.FluidImage(stack.liquid.uiIcon, stack.amount * 60f))
                    .pad(2f);
            if (isInput) iconCell.left();
            else iconCell.right();
            if (showNameTooltip)
        ...
        // Power
        if (entry.power > 0f) {
            Cell<TEMod.MultiCraftLib.ui.PowerImage> iconCell = mat.add(new TEMod.MultiCraftLib.ui.PowerImage(entry.power * 60f))
                    .pad(2f);
            if (isInput) iconCell.left();
            else iconCell.right();
            if (showNameTooltip)
        ...
        //Heat
        if (entry.heat > 0f) {
            Cell<TEMod.MultiCraftLib.ui.HeatImage> iconCell = mat.add(new TEMod.MultiCraftLib.ui.HeatImage(entry.heat))
                    .pad(2f);
            if (isInput) iconCell.left();
            else iconCell.right();
            if (showNameTooltip)
        ...
        for (PayloadStack stack : entry.payloads) {
            Cell<TEMod.MultiCraftLib.ui.PayloadImage> iconCell = mat.add(new TEMod.MultiCraftLib.ui.PayloadImage(stack.item.uiIcon, stack.amount))
                    .pad(2f);
            if (showNameTooltip)
                iconCell.tooltip(stack.item.localizedName);
            if (isInput) iconCell.left();
